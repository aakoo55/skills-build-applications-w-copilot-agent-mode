...
export default function Teams() {
  ...
  const renderTable = (data) => {
    if (!data || data.length === 0) return <div className="text-muted">No teams found</div>;
  ...
  return (
    <div className="card mb-4">
      <div className="card-body">
        ...
        {loading && <div className="text-secondary">Loading teams...</div>}
        {error && <div className="text-danger">Error loading teams: {String(error)}</div>}
