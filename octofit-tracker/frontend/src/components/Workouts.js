...
export default function Workouts() {
  ...
  const endpoint = process.env.REACT_APP_CODESPACE_NAME
    ...
    : (window.REACT_APP_API_BASE || 'http://localhost:8000/api') + '/workouts/';
  ...
  const renderTable = (data) => {
    if (!data || data.length === 0) return <div className="text-muted">No workouts found</div>;
  ...
  return (
    <div className="card mb-4">
      <div className="card-body">
        ...
        {loading && <div className="text-secondary">Loading workouts...</div>}
        {error && <div className="text-danger">Error loading workouts: {String(error)}</div>}
